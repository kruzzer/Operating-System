...
int main(int argc , char * argv[])
{
    ...
    while(1) 
    {
        ...
        else
        {
            ...
            get_matrix() ;
            matrix[1-queue_id][consumer_no-1] = 1 ;
            update_matrix() ;
            print_matrix() ;
            ...
            down(semid_queue[1-queue_id] , mutex) ;
            ...
            get_matrix() ;
            matrix[1-queue_id][consumer_no-1] = 2 ;
            update_matrix() ;
            print_matrix() ;
            ...
            printf("Giving up control of file\n") ;
             ...
             in_the_buffer = semctl(semid_queue[1-queue_id] , no_of_messages , GETVAL , 0) ;
            
            if(in_the_buffer > 0)
            {
                memset(buffer.mtext,0,2) ;
                if(msgrcv(msgid[1-queue_id],&buffer,(size_t)(2),0,IPC_NOWAIT) > 0)
                {
                    int recieve = (int)(buffer.mtext[0]) ;
                    printf("Consumer with id : %d read %d from queue with id : %d\n",consumer_no,recieve ,1-queue_id) ;   
                    up(semid_queue[1-queue_id],total_messages_deleted) ;
                    down(semid_queue[1-queue_id] , no_of_messages) ;
                }
                else
                {
                     perror("Error in msg recieving") ;
            ...
            get_matrix() ;
            matrix[1-queue_id][consumer_no-1] = 0 ;
            update_matrix() ;
            print_matrix() ;
            ...
            up(semid_queue[1-queue_id] , mutex) ;
